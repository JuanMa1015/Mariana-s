...
function Row({ index, style, data }: ListChildProps) {
  const { procesos, abierto, toggleExpand, onOpenDetalle, handleCopy, handleDelete, abrirEditor } = data
          ...
          <tbody>
              ...
              <td className="px-5 py-4 align-middle">
                {p.categoria ? (
                  <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold ${
                    p.categoria === "Trabajo" ? "border-sky-200 bg-sky-50 text-sky-700" : p.categoria === "Consultorio" ? "border-amber-200 bg-amber-50 text-amber-700" : "border-violet-200 bg-violet-50 text-violet-700"
                  }`}>
            ...
            <tr>
              <td colSpan={8} className="border-b border-violet-50 bg-gradient-to-b from-violet-50/50 to-white px-6 pb-6 pt-0">
                <div className="mt-1 rounded-2xl border border-violet-100 bg-white p-5 shadow-sm">
...
export default function TablaProcesos({ procesos, onOpenDetalle, onDelete }: Props) {
  ...
  const toggleExpand = useCallback((llave: string) => {
    setAbierto(prev => prev === llave ? null : llave)
  }, [])

  const handleDelete = useCallback(async (e: React.MouseEvent, llave: string) => {
    e.stopPropagation()
    toast((t) => (
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-2">
          <button className="flex-1 rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-rose-700 transition"
            onClick={async () => {
              toast.dismiss(t.id)
          ...
          >Sí, eliminar</button>
          <button className="flex-1 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition"
  ...
  const itemSize = useCallback((index: number) => {
    return abierto === procesos[index]?.llave_proceso ? ROW_HEIGHT + EXPANDED_HEIGHT : ROW_HEIGHT
  }, [abierto, procesos])
  ...
  return (
    <>
      <div className="h-full flex flex-col">
        <table className="w-full min-w-[960px] text-left text-sm border-separate border-spacing-0 shrink-0">
          ...
          <div className="w-full max-w-md rounded-3xl border border-violet-100 bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <input value={editForm.llave_proceso} onChange={(e) => setEditForm({ ...editForm, llave_proceso: e.target.value.replace(/\D/g, "") })} className="w-full rounded-2xl border border-violet-200 bg-violet-50/30 px-4 py-3 text-sm outline-none transition focus:border-violet-400 focus:ring-4 focus:ring-violet-100" maxLength={23} inputMode="numeric" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">Categoría</label>
                <select value={editForm.categoria} onChange={(e) => setEditForm({ ...editForm, categoria: e.target.value })} className="w-full rounded-2xl border border-violet-200 bg-violet-50/30 px-4 py-3 text-sm outline-none transition focus:border-violet-400 focus:ring-4 focus:ring-violet-100">
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button onClick={() => setEditando(null)} className="rounded-2xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">Cancelar</button>
              <button onClick={guardarEdicion} className="rounded-2xl bg-violet-500 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-violet-600 active:scale-95">Guardar</button>
            </div>
          </div>
        </div>
      )}
